...
proc buildStatLines(state: GameState): seq[string] =
  ...
  @[
    ...
    &"Level: {p.level}",
    &"{anyItem(\"currency\").displayName}: {countItem(state, \"currency\")}",
    "=-=-=-=",
    &"HP: {p.health.int:>3} / {p.maxHealth.int:>3} | {statBar(p.health, p.maxHealth)}",
    &"Stamina: {p.stamina.int:>3} / {p.maxStamina.int:>3} | {statBar(p.stamina, p.maxStamina)}",
    &"Focus: {p.focus.int:>3} / {p.maxFocus.int:>3} | {statBar(p.focus, p.maxFocus)}",
    "=-=-=-=",
